...
// Map a user-facing URL path to its pre-built markdown asset path.
// Returns null if no markdown representation is available.
function markdownAssetPath(pathname: string): string | null {
  const p = pathname.replace(/\/+$/, '') || '/';
  if (p === '/') return '/index.md';
  if (p === '/blog') return '/blog/index.md';
  if (p.startsWith('/blog/')) return `${p}.md`;
  return null;
}
...
export const onRequest = async (context: PagesCtx): Promise<Response> => {
  ...
  // ── Normal response; add Link header on HTML ─────────────────────────
